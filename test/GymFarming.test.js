...
describe("GymFarming contract: ", function () {
    before("Before All: ", async () => {
        ...
        this.provider = await ethers.getContractAt("ILiquidityProvider", "0x2B1C93fFfF55E2620D6fb5DaD7D69A6a468C9731", accounts.caller);
    ...
    describe("Set function: ", async () => {
        ...
        it("Should revert if invalid pool", async () => {
            await expect(this.gymFarming.set(0, poolAllocPoint2, true))
    ...
    describe("pendingReward function: ", async () => {
        ...
        it("PendingReward should equal ExpectedGym: ", async () => {
            ...
            expect(pendingReward).to.equal(await this.gymFarming.getMultiplier(this.startBlock, this.startBlock + blockToAdvance));

            harvesttx = await this.gymFarming.connect(accounts.caller).harvest(pid)
            expect(await this.gym.balanceOf(accounts.caller.address)).to.equal(await this.gymFarming.getMultiplier(this.startBlock, this.startBlock + blockToAdvance + 1));
        })

        it("PendingReward should equal ExpectedGym (2 signers in 1 pool): ", async () => {
            const pid = await this.gymFarming.poolLength();
            ...
            let pendingRewardCaller = await this.gymFarming.pendingReward(pid, accounts.caller.address)
            ...
            harvesttx = await this.gymFarming.connect(accounts.caller).harvest(pid);
            expect(await this.gym.balanceOf(accounts.caller.address)).to.equal(pendingRewardCaller.add((await this.gymFarming.rewardPerBlock()).div(2)));
            ...
            expect(await this.gym.balanceOf(accounts.chugun.address)).to.equal((await this.gymFarming.getMultiplier(log2.blockNumber, harvesttx.blockNumber)).div(2));
        })

        it("PendingReward should equal ExpectedGym(4 signers in 2 pools): ", async () => {
            ...
            expect(await this.gym.balanceOf(accounts.caller.address)).to.equal((await this.gymFarming.getMultiplier(log3.blockNumber, harvesttx.blockNumber)).div(5).add(pendingReward1));

            harvesttx = await this.gymFarming.connect(accounts.chugun).harvest(pid1);
            expect(await this.gym.balanceOf(accounts.chugun.address)).to.equal((await this.gymFarming.getMultiplier(log3.blockNumber, harvesttx.blockNumber)).div(5).add(pendingReward2));

            harvesttx = await this.gymFarming.connect(accounts.vzgo).harvest(pid1);
            expect(await this.gym.balanceOf(accounts.vzgo.address)).to.equal((await this.gymFarming.getMultiplier(log3.blockNumber, harvesttx.blockNumber)).div(10).add(pendingReward3));

            vzgoGym = await this.gym.balanceOf(accounts.vzgo.address);

            harvesttx = await this.gymFarming.connect(accounts.vzgo).harvest(pid2);
            expect((await this.gym.balanceOf(accounts.vzgo.address)).sub(vzgoGym)).to.equal((await this.gymFarming.getMultiplier(log5.blockNumber, harvesttx.blockNumber)).div(4).add(pendingReward4));

            harvesttx = await this.gymFarming.connect(accounts.grno).harvest(pid2);
            expect(await this.gym.balanceOf(accounts.grno.address)).to.equal((await this.gymFarming.getMultiplier(log5.blockNumber, harvesttx.blockNumber)).div(4).add(pendingReward5));
        })
    })
    ...
    describe("setRewardPerBlock function", async () => {
        ...
        it("Should change rewardPerBlock:", async () => {
            let rewardPerBlock = await this.gymFarming.rewardPerBlock();
            ...
            expect(Math.floor(ethers.BigNumber.from(await this.gymFarming.rewardPerBlock()))).to.equal(Math.floor(rewardPerBlock * variables.gymFarming[0] / 1e12))

            await advanceBlockTo(this.startBlock + 41)

            await this.gymFarming.setRewardPerBlock()

            expect(Math.floor(ethers.BigNumber.from(await this.gymFarming.rewardPerBlock()))).to.equal(Math.floor(rewardPerBlock * (variables.gymFarming[0] ** 2) / (1e12 ** 2)))

            await advanceBlockTo(this.startBlock + 62)

            await this.gymFarming.setRewardPerBlock()

            expect(Math.floor(ethers.BigNumber.from(await this.gymFarming.rewardPerBlock()))).to.equal(Math.floor(rewardPerBlock * (variables.gymFarming[0] ** 3) / (1e12 ** 3)))
    ...
    describe("Add function: ", async () => {
        ...
        it("Should add new pool for deposit: ", async () => {
            const pid = await this.gymFarming.poolLength();
            let tx = await this.gymFarming.add(poolAllocPoint2, this.testLp.address, true)
    ...
    describe("Deposit function: ", async () => {
        ...
        it("Should deposit in pool: ", async () => {
            ...
            let accountLp = await this.testLp.balanceOf(accounts.caller.address)
    ...
    describe("Withdraw function: ", async () => {
        ...
        it("Withdraw lps from pool: ", async () => {
            ...
            let accountLp = await this.testLp.balanceOf(accounts.caller.address)
            let contractLp = await this.testLp.balanceOf(this.gymFarming.address)
            await this.testLp.connect(accounts.caller).approve(this.gymFarming.address, amount)
            await this.gymFarming.connect(accounts.caller).deposit(pid, amount)
            await advanceBlock()
    ...
    describe("HarvestAll function: ", async () => {
        ...
        it("Should HarvestAll: Call HarvestAll function to get all assets at ones ", async () => {
            ...
            expect((await this.gymFarming.userInfo(0, accounts.vzgo.address)).rewardDebt).to.not.equal(ethers.constants.Zero)
            expect((await this.gymFarming.userInfo(1, accounts.vzgo.address)).rewardDebt).to.not.equal(ethers.constants.Zero)
        })
    })

    describe("SpeedStake function: ", async () => { //only work when forked
        ...
        before("Before: ", async () => {
            ...
            await router.connect(accounts.holder).addLiquidityETH(this.gym.address, ethers.utils.parseEther("10"), 0, 0, accounts.holder.address, new Date().getTime() + 20, {
        ...
        it("Should deposit in pool: ", async () => {
            await expect(() => this.gymFarming.connect(accounts.holder).speedStake(0, 0, 0, 0, 0, new Date().getTime() + 20, {
                    value: ethers.utils.parseEther("10")
                }))
                .to.changeTokenBalances(
                    this.gym, [accounts.holder], [
                        0,
                    ]
                )
        ...
        it("Should deposit in pool: ", async () => {
            ...
            await expect(() => this.gymFarming.connect(accounts.holder).speedStake(0, ethers.utils.parseEther("10"), 0, 0, 0, new Date().getTime() + 20, {
                    value: ethers.utils.parseEther("10")
                }))
                .to.changeTokenBalances(
                    this.gym, [accounts.holder], [
    ...
    describe("ClaimAndDeposit function: ", async () => { //only work when forked
        let router, factory, lpToken;
        before("Before: ", async () => {
            router = await ethers.getContractAt("IPancakeRouter02", "0x10ED43C718714eb63d5aA57B78B54704E256024E");
            factory = await ethers.getContractAt("IPancakeFactory", await router.factory())
        ...
        it("Should claimA in pool: ", async () => {
            let tx = await this.gymFarming.connect(accounts.holder).speedStake(0, 0, 0, 0, 0, new Date().getTime() + 20, {
                value: ethers.utils.parseEther("10")
            });
