...
describe("GymVaultsStrategyAlpacaBUSD contract: ", function () { // only working when forked
    before("Before All: ", async () => {
        ...
        await deployments.deploy("GymFarming", {
            from: accounts.deployer.address,
            args: [deploymentArgs.bank, deploymentArgs.rewardToken, deploymentArgs.rewardPerBlock, deploymentArgs.startBlock],
            log: true,
            deterministicDeployment: false
        })
        ...
        let signer = await ethers.getSigner("0xf9211FfBD6f741771393205c1c3F6D7d28B90F03")
        ...
        await this.router.connect(accounts.holder)
            .addLiquidityETH(this.gymToken.address, ethers.utils.parseEther("1000"), 0, 0, this.farming.address, new Date().getTime() + 20, {
        ...
        await this.router.connect(accounts.holder).addLiquidity(this.gymToken.address, this.busd.address, ethers.utils.parseEther("1000"), ethers.utils.parseEther("1000"), 0, 0, accounts.holder.address, new Date().getTime() + 20, {
            gasLimit: 5000000
        })
    })
    ...
    describe("Deposit function: ", async () => {
        ...
        it("Should accept deposit from user: ", async () => {
            await this.busd.connect(accounts.holder).approve(this.bank.address, ethers.utils.parseEther("100"))

            await this.bank.connect(accounts.holder).deposit(1, ethers.utils.parseEther("100"), 1, 0, new Date().getTime() + 20)
    ...
    describe("Claim function: ", async () => {
        ...
        it("Should accept claim from user: ", async () => {
            await this.busd.connect(accounts.holder).approve(this.bank.address, ethers.utils.parseEther("0.1"))

            let tx = await this.bank.connect(accounts.holder).deposit(1, ethers.utils.parseEther("0.1"), 1, 0, new Date().getTime() + 20)

            await advanceBlockTo(tx.blockNumber + 100)

            let pending = await this.bank.pendingReward(1, accounts.holder.address);

            await expect(() => this.bank.connect(accounts.holder).claim(1)).to.changeTokenBalances(this.gymToken, [accounts.holder, this.bank], [
    ...
    describe("Withdraw function: ", async () => {
        ...
        it("Should accept withdraw from user: ", async () => {
            await this.busd.connect(accounts.holder).approve(this.bank.address, ethers.utils.parseEther("0.1"))
            ...
            await expect(() => this.bank.connect(accounts.holder).withdraw(1, ethers.utils.parseEther('0.04'))).to.changeTokenBalances(this.busd, [accounts.holder], [
                ethers.utils.parseEther('0.036')
            ])
        })
    })

    describe("ClaimAndDeposit function: ", async () => {
        before("Before: ", async () => {
            this.snapshotStart = await network.provider.request({
                method: "evm_snapshot",
                params: [],
        ...
        it("Should accept claimAndDeposit from user: ", async () => {
            await this.busd.connect(accounts.holder).approve(this.bank.address, ethers.utils.parseEther("0.1"))
